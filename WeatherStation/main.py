...
def do_admin_login():
    if request.form['password'] == '1398esp8266' and request.form['username'] == 'admin':
...
def postWeatherData():
    if request.method == 'GET':
        ...
        humidiy = request.args.get('Hum')
        pressure = request.args.get('Pres')
        date = request.args.get('Date')
        time = request.args.get('Time')

        print("New Weather Data. T:" + temperature + " H:" + humidiy + " P:" + pressure + " Date:" + date + " Time:" + time)
        datetime = date + " " + time
        insertResult = saveWeatherData(temperature, humidiy, pressure, datetime)
